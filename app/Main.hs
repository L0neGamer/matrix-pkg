...
m3 = fromJust $ setAtMatrix 1 0 1 $ fromJust $ setAtMatrix 0 0 0 $ constructFrom cantorPairing v3 v3
...
main = do
    ...
    printMatrix $ translation3Ex *.* (transpose $ singleton (0 .:: 0 .:: 0 .:: singleton 1))
    printMatrix $ scale2Ex *.* (transpose $ singleton (1 .:: 1 .:: singleton 1))
    printMatrix $ translation2Ex *.* rotation2Ex *.* scale2Ex *.* (transpose $ singleton (1 .:: 1 .:: singleton 1))
    printMatrix $ translation2Ex *.* rotation2Ex *.* scale2Ex *.* (rotation2 (-pi/2))
    print $ v1 `dotProd` v1
