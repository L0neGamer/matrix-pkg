...
m3 =
  fmap fromInteger
    $ setAtMatrix (FSucc FZero) FZero 1
...
translate2 a b =
  Mat $ (1 :+ 0 :+ singleton a) :+ (0 :+ 1 :+ singleton b) :+ singleton
    (0 :+ 0 :+ singleton 1)

scale2 :: Num a => a -> a -> Matrix Three Three a
scale2 a b =
  Mat $ (a :+ 0 :+ singleton 0) :+ (0 :+ b :+ singleton 0) :+ singleton
    (0 :+ 0 :+ singleton 1)

rotation2 :: Floating a => a -> Matrix Three Three a
rotation2 a =
  Mat $ (c :+ (-s) :+ singleton 0) :+ (s :+ c :+ singleton 0) :+ singleton
    (0 :+ 0 :+ singleton 1)
 where
  c = cos a
  s = sin a
...
main = do
  ...
  print $ toList $ crossProd (1 :+ 0 :+ singleton (0 :: Integer))
                             (0 :+ 1 :+ singleton 0)
  printMatrix m4
  print $ det m4
  printMatrix $ m5
